...
def create_agentic_reasoning_context(df: pd.DataFrame) -> dict:
    ...
    #generate data patterns for LLM to analyze
    data_insights = {
        "temporal_patterns": {},
        "value_distributions": {},
        "correlation_hints": {},
    ...
    #correlation hints (encourage our llm to find relationships)
...
def generate_investigative_prompts(df: pd.DataFrame) -> list:
    """generate context specific investigative q's for the LLM to consider."""
...
def build_prompt(df: pd.DataFrame, chat_history):
    """return a system prompt dict for LLM ChatCompletion with agentic reasoning."""
